	...
	protected def doList(params) {
		...
		*/	  			  
			...
			if (params.cognomeRichiedente) {
				def restriction = "exists (select id from detail_pers per where per.kennung_id = 1 AND per.ba_id = {alias}.ba_id AND per.familienname = '${params.cognomeRichiedente.toUpperCase().replaceAll('\'', '')}'"
			...
			if (params.cognomeProgettista) {
				def restriction = "exists (select id from detail_pers per where per.kennung_id = 3 AND per.ba_id = {alias}.ba_id AND per.familienname = '${params.cognomeProgettista.toUpperCase().replaceAll('\'', '')}'"
